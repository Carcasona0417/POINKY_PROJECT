# Please put something here

1. Notification for Dashboard
2. Instead Monthly Financial Overview --> Yearly

3. REMNIDERS UI - contains reminders/TODOLIST UI (need to integrate this to the dashboard)
4. Compile Poinky - contains the compilation of all the the UI and been cleaned
 
 mikaaaaaaaaaaaaaa
 1. Pie Chart in dashboard - expenses (refer to figma) - ADDED
 2. Put eye slash in every ui that's lacking - DONE PROFILE SETTINGS
- FORGOTPASS
 3. Check confirmation messages
 4. If possible, add error bootstrap messages in input boxes (all)
 5. Farm improve layout and change colors
 6. Notification modal fix - DONE
 7. add animation headers 
 (all ui)
 profile settings - done
 dashboard
 rem
 farm
 expenses
 8. update profile button in dashboard
 9. change colors 
 index.html
 login
 10. put dashboardnew.html dashnew.css dashnew.js in Mini-Capstone folder

 # CHANGES HTML AND JS (DASHBAORD) ---- FOR MORE INFORMATION JUST GO TO BACKEND NOTES!!
 - Legend-items for medicines change into medical (JUST TEXT/DISPLAY)
 - Added ASYNC FUNCTION and FETCH data 
 - After fetching data Calculates all the fetched data from the database
 - Feed expenses was deleted from both html and js along with the legend-items
 - I deleted the extra codes in the dash.js and eliminate double code
 - It is fully integrated except for the notification
